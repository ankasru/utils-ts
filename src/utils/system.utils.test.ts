...
describe('clipboard', () => {
    ...
    vi.stubGlobal('window', {
      navigator: {
        clipboard: {
          ...
          readText: vi
            .fn()
            .mockImplementation(async () => await Promise.resolve('test')),
        },
      },
    });
  });
  ...
  test('clipboard error', async () => {
    window.navigator.clipboard.readText = vi
      .fn()
      .mockImplementation(async () => await Promise.reject(new Error('error')));
    window.navigator.clipboard.writeText = vi
      .fn()
      .mockImplementation(async () => await Promise.reject(new Error('error')));
    expect(await readFromClipboard()).toBeFalsy();
    expect(await writeToClipboard('123')).toBeFalsy();
