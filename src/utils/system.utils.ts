...
export function isSmallTablet(): boolean {
  return (
    window.innerWidth >= SMALL_TABLET_SIZE && window.innerWidth <= TABLET_SIZE
  );
}
...
export function canUseDom(): boolean {
  return typeof window !== 'undefined' && window.document instanceof Document;
}
